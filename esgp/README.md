...
**封测阶段模型接口提交格式说明**
请参赛选手先将api信息按照下述json格式填写，再将json文件压缩成zip文件进行提交。评测方在提交过程中会根据选手提交的api的url进行一次简单的调用，用于判断其返回格式是否为上述响应格式，格式正确则提交成功，返回“https://api.example.com/model已提交，后续评测中”信息，否则提交失败，请仔细检查本地部署情况，重新提交。
```json
...
请确保测试期间（**2024.11.26下午13：00-2024.11.27下午18:00**）您的具备稳定性和可靠性，以便在测试时能够正常访问和使用。
